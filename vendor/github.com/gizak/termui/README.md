...
Go modules: It is not necessary to `go get` termui, since Go will automatically download any imported dependencies. Do note that you have to include `` at the end of the URL and before any sub-packages in the termui import statements as shown in the 'Hello World' example below.

Dep: termui can be added with Dep using `dep ensure -add github.com/gizak/termui` but the `` needs to be *removed* from all import statements.
